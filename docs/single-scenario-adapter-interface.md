...
| `tests/test_single_live_smoke_real_adapter_placeholder.py` | Contract tests proving the placeholder is inert, including Stage 112 exact error-message coverage. |
...
Calling `build_real_single_live_result_payload()` raises `SingleLiveSmokeRealAdapterNotImplementedError` with the exact message `Real single-scenario adapter is not implemented. Use the fake adapter path.`.
