...
Calling `build_real_single_live_result_payload()` raises `SingleLiveSmokeRealAdapterNotImplementedError`, a `NotImplementedError` subclass that is also catchable through the base `NotImplementedError` type, with the exact message `Real single-scenario adapter is not implemented. Use the fake adapter path.`, without mutating the input scenario, including nested scenario data, before requiring a specific scenario shape, without inspecting scenario fields, without iterating scenario fields, before requiring a dict-like scenario type, before validating a `None` scenario, without calling `repr()` on the scenario, without evaluating scenario truthiness, and without checking scenario length.
