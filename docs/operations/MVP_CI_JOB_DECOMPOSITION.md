...
`MVP Contract Checks` historically ran the entire offline MVP validation matrix in one `mvp-contracts` job. The suite is intentionally broad, but an early browser-contract failure can prevent unrelated later contracts from producing evidence and can obscure subsystem ownership.
...
The workflow keeps the top-level name `MVP Contract Checks` and assigns the existing commands to eight independent fail-closed jobs:
...
A final `mvp-contracts` compatibility aggregator depends on all eight jobs and fails unless every dependency is `success`. This preserves the established `MVP Contract Checks / mvp-contracts` check identity for branch-protection consumers while allowing independent contract domains to execute concurrently.
...
3. every pre-decomposition named test/evidence/whitespace step remains present exactly once across the domain jobs;
