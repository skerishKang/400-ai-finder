...
- `.github/**/*.md` — no `.github/**/*.md` existed in the `main` audit baseline. This branch added `.github/pull_request_template.md` as a new file. The `docs/artifacts/1199-*-document-inventory.md` and `docs/product/clone-visual-fidelity-and-promotion-policy.md` are new files added by this branch; the inventory audit itself was performed on the pre-change `main` branch state.
