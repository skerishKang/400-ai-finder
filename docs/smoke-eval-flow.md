...
Stage 40~58에서 구축한 400-ai-finder의 오프라인 smoke eval 흐름을 정리한 문서입니다.

이 평가 흐름은 기본적으로 외부 네트워크, live provider, Firecrawl, 실제 app pipeline 호출을 수행하지 않습니다. 이미 준비된 시나리오 매트릭스, 응답 fixture, demo/pipeline 결과 JSON, mock live smoke response builder, 또는 live smoke artifact fixture를 사용해 품질 게이트를 반복 검증합니다.
...
Stage 58 이후에는 다음 중 하나로 확장할 수 있습니다.

1. 실제 provider/fetch 호출 전용 `--live` opt-in 구현
2. live 실행 결과를 Stage 56/57 artifact shape로 저장하는 writer 추가
3. live artifact writer 결과를 Stage 58 helper로 재평가하는 roundtrip 추가
4. Admin dashboard에 eval result import/export 기능 추가
5. demo/pipeline result fixture builder 보강

권장 순서는 실제 live 호출을 바로 넓게 붙이는 것이 아니라, 현재의 mock dry-run, pipeline export eval dry-run, live artifact export eval dry-run 경로를 기준선으로 유지한 상태에서 provider/fetch opt-in, 실행 결과 artifact writer, judge 연결을 각각 별도 Stage로 분리하는 것입니다.
