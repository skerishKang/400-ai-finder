...
Stage 40~52에서 구축한 400-ai-finder의 오프라인 smoke eval 흐름을 정리한 문서입니다.
...
Stage 52 이후에는 다음 중 하나로 확장할 수 있습니다.

1. 실제 provider/fetch 호출 전용 `--live` opt-in 구현
2. live 실행 결과를 Stage 42 response judge에 연결
3. live 결과 저장/재평가용 JSON export 추가
4. Admin dashboard에 eval result import/export 기능 추가
5. demo/pipeline result fixture builder 보강

권장 순서는 실제 live 호출을 바로 넓게 붙이는 것이 아니라, 현재의 mock dry-run 경로를 기준선으로 유지한 상태에서 provider/fetch opt-in, 실행 결과 export, judge 연결을 각각 별도 Stage로 분리하는 것입니다.
