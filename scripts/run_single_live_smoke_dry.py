...
result payload with deterministic dummy data, and writes it through the Stage
60 artifact writer.
"""
...
def _answer_keyword(scenario: dict[str, Any]) -> str:
...
def _source_title(scenario: dict[str, Any], keyword: str) -> str:
...
def build_single_scenario_payload(scenario: dict[str, Any]) -> dict[str, Any]:
    ...
    keyword = _answer_keyword(scenario)

    needs_source = isinstance(min_sources, int) and min_sources > 0
    if needs_source:
        sources = [
            {
                "title": _source_title(scenario, keyword),
