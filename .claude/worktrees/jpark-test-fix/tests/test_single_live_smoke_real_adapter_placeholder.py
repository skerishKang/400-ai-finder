...
class _RaisingGetattributeScenario:
...
class _RaisingClassLookupScenario:
    @property
    def __class__(self) -> type:
        raise AssertionError("scenario class lookup should not be called")


class _RaisingDictLookupScenario:
    @property
...
class _RaisingModuleLookupScenario:
    @property
...
class _RaisingWeakrefLookupScenario:
    def __getattr__(self, name: str) -> object:
        if name == "__weakref__":
            raise AssertionError("scenario weakref lookup should not be called")
        raise AttributeError(name)
...
class _RaisingDeepcopyScenario:
...
class _RaisingReduceProtocolScenario:
    def __reduce__(self) -> object:
        raise AssertionError("scenario reduce protocol lookup should not be called")


class _RaisingGetstateProtocolScenario:
    def __getstate__(self) -> object:
        raise AssertionError("scenario getstate protocol lookup should not be called")


class _RaisingGetnewargsExProtocolScenario:
    def __getnewargs_ex__(self) -> object:
...
def test_stage180_real_adapter_placeholder_does_not_call_scenario_class_lookup() -> None:
    with pytest.raises(SingleLiveSmokeRealAdapterNotImplementedError):
        build_real_single_live_result_payload(_RaisingClassLookupScenario())  # type: ignore[arg-type]
...
def test_stage168_real_adapter_placeholder_does_not_call_scenario_reduce_protocol() -> None:
    with pytest.raises(SingleLiveSmokeRealAdapterNotImplementedError):
        build_real_single_live_result_payload(_RaisingReduceProtocolScenario())  # type: ignore[arg-type]


def test_stage170_real_adapter_placeholder_does_not_call_scenario_getstate_protocol() -> None:
    with pytest.raises(SingleLiveSmokeRealAdapterNotImplementedError):
        build_real_single_live_result_payload(_RaisingGetstateProtocolScenario())  # type: ignore[arg-type]
