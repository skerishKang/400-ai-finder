...
export function classifyAction(question) {
  ...
  for (const rule of ACTION_RULES) {
    if (rule.terms.some((term) => normalized.includes(term.toLowerCase()))) return rule.action;
  }
  if (hasIndirectLitterSignal(normalized)) return 'litter_ai_assist';
  return 'none';
}
...
function hasIndirectLitterSignal(normalized) {
  ...
  if (!(hasTarget && hasDumping && hasIntent)) return false;
  return !LITTER_COLLECTION_COMPLAINT_CUES.some((cue) => normalized.includes(cue));
}
...
function buildCanonicalSnapshotContext(action) {
  ...
  if (snapshot.snapshot_kind === 'official_content_page') {
    ...
    return {
      ok: true,
      evidence: [`[공식 스냅샷 ${snapshot.snapshot_id}]`, `페이지: ${source.title}`, `공식 URL: ${source.url}`, `공식 페이지 업데이트 표시: ${source.source_updated_at}`, `캡처된 공식 본문:\n${officialText}`].join('\n'),
      sources: [{ title: source.title, url: source.url, official: true, snapshot_id: snapshot.snapshot_id, canonical_sha256: snapshot.canonical_sha256, captured_at: source.captured_at, verified_at: source.verified_at, source_updated_at: source.source_updated_at }],
  ...
  const rows = snapshot.page.rows.map((row, index) => `${index + 1}. ${row.department} | ${row.team} | ${row.position} | ${row.phone} | ${row.duty}`);
  const source = snapshot.source;
  const contactSource = snapshot.representative_contact_source;
  const contact = snapshot.representative_contact;
  const sources = [
    { title: source.title, url: source.url, official: true, snapshot_id: snapshot.snapshot_id, canonical_sha256: snapshot.canonical_sha256, captured_at: source.captured_at, verified_at: source.verified_at, source_updated_at: source.source_updated_at },
    { title: contactSource.title, url: contactSource.url, official: true, snapshot_id: snapshot.snapshot_id, canonical_sha256: snapshot.canonical_sha256, captured_at: contactSource.captured_at, verified_at: contactSource.verified_at, source_updated_at: contactSource.source_updated_at },
  ];
  return {
    ok: true,
    evidence: [`[공식 스냅샷 ${snapshot.snapshot_id}]`, `페이지: ${source.title}`, `공식 URL: ${source.url}`, `공식 페이지 최근업데이트: ${source.source_updated_at}`, `부서 대표전화: ${contact.phone}`, `FAX: ${contact.fax}`, `${snapshot.page.content_heading} / ${snapshot.page.count_label}`, columnLabels, ...rows].join('\n'),
    sources,
    sourceUrl: source.url,
    searchQueries: [],
    freshnessState: 'official_snapshot',
...
function buildSnapshotUnavailableContext(action) {
  return { ok: false, evidence: '', sources: [], sourceUrl: '', searchQueries: [], freshnessState: 'snapshot_unavailable', capturedAt: '', verifiedAt: '', routeId: '', pageId: '', snapshotId: '', canonicalSha256: '', action };
}
...
export function normalizeProviderOrder(value) {
  const raw = typeof value === 'string' && value.trim() ? value : DEFAULT_PROVIDER_ORDER.join(',');
  const order = [];
...
function providerConfig(provider, env, requestHostnameValue) {
  ...
  if (endpointResolution.error) {
    const key = provider === 'hy3' ? envText(env, 'KILOCODE_API_KEY', '') : envText(env, 'GEMINI_API_KEY', '');
    return { provider, error: endpointResolution.error, endpointErrorReason: endpointResolution.reason, key, model: '', endpoint: '', apiStyle: 'openai' };
  }
  if (provider === 'hy3') {
    return { provider, key: envText(env, 'KILOCODE_API_KEY', ''), model: envText(env, 'HY3_MODEL', PROVIDER_DEFAULTS.hy3.model), endpoint: endpointResolution.endpoint, apiStyle: 'openai' };
  }
  const style = envText(env, 'GEMINI_API_STYLE', PROVIDER_DEFAULTS.gemini.apiStyle).toLowerCase();
  return { provider: 'gemini', key: envText(env, 'GEMINI_API_KEY', ''), model: envText(env, 'GEMINI_MODEL', PROVIDER_DEFAULTS.gemini.model), endpoint: endpointResolution.endpoint, apiStyle: style === 'interactions' ? 'interactions' : 'openai' };
}

function formatSeoulTime(date) {
  return new Intl.DateTimeFormat('ko-KR', { timeZone: 'Asia/Seoul', year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }).format(date);
}

function targetLanguageInstruction(locale) {
  const target = normalizeLocale(locale);
  switch (target) {
    case 'en': return ['Selected locale: en.', 'Write ALL resident-facing explanatory prose in clear, natural English (2–5 sentences).', 'Do not write the explanation in Korean, Thai, Vietnamese, or Indonesian.', 'Only official Korean department names, service names, addresses, phone numbers, and URLs may remain in their official form.', 'Do not disguise a full Korean explanation as an official proper noun.'].join(' ');
    case 'vi': return ['Selected locale: vi.', 'Write ALL resident-facing explanatory prose in natural Vietnamese (tiếng Việt), 2–5 sentences.', 'Do not write the explanation in Korean, English-only, or Thai.', 'Only official Korean department names, service names, addresses, phone numbers, and URLs may remain in their official form.'].join(' ');
    case 'th': return ['Selected locale: th.', 'Write ALL resident-facing explanatory prose in natural Thai (ภาษาไทย), 2–5 sentences.', 'Do not write the explanation in Korean or English-only Latin prose.', 'Only official Korean department names, service names, addresses, phone numbers, and URLs may remain in their official form.'].join(' ');
    case 'id': return ['Selected locale: id.', 'Write ALL resident-facing explanatory prose in natural Indonesian (bahasa Indonesia), 2–5 sentences.', 'Do not write the explanation in Korean, English-only, or Thai.', 'Only official Korean department names, service names, addresses, phone numbers, and URLs may remain in their official form.'].join(' ');
    case 'ko': default: return ['Selected locale: ko.', '주민에게 바로 도움이 되도록 자연스러운 한국어 설명문 2~5문장으로 답하세요.', '설명문을 영어·태국어 위주로 쓰지 마세요.', '공식 한국어 부서명, 서비스명, 전화번호, 주소, URL은 원문을 유지할 수 있습니다.'].join(' ');
  }
}
...
function buildGroundedPrompt(question, currentTime, officialContext, locale, rejectedDraft) {
  const base = rejectedDraft ? buildCorrectiveSystemPrompt(currentTime, officialContext, locale, rejectedDraft) : buildSystemPrompt(currentTime, officialContext, locale);
  return [base, '', 'Confirm current facts with the Google search tool when available.', 'Prefer bukgu.gwangju.kr, search.bukgu.gwangju.kr, and public-sector domains for Buk-gu administrative questions.', 'When possible, search site:bukgu.gwangju.kr or site:search.bukgu.gwangju.kr first.', '', `Resident question: ${question}`].join('\n');
}

function isOfficialUrl(value) {
  try {
    const hostname = new URL(value).hostname.toLowerCase();
    return hostname === 'bukgu.gwangju.kr' || hostname.endsWith('.bukgu.gwangju.kr') || hostname.endsWith('.gwangju.kr') || hostname.endsWith('.go.kr') || hostname.endsWith('.gov.kr');
  } catch (_) { return false; }
}

function safeSource(annotation) {
  if (!annotation || annotation.type !== 'url_citation' || typeof annotation.url !== 'string') return null;
  try {
    const url = new URL(annotation.url);
    if (url.protocol !== 'https:' && url.protocol !== 'http:') return null;
    return { title: typeof annotation.title === 'string' && annotation.title.trim() ? annotation.title.trim().slice(0, 160) : url.hostname, url: url.toString(), official: isOfficialUrl(url.toString()) };
  } catch (_) { return null; }
}

export function parseGroundedInteraction(data) {
  const textParts = []; const sources = []; const searchQueries = []; const seenSources = new Set();
  const steps = data && Array.isArray(data.steps) ? data.steps : [];
  for (const step of steps) {
    if (step && step.type === 'google_search_call') {
      const queries = step.arguments && Array.isArray(step.arguments.queries) ? step.arguments.queries : [];
      for (const query of queries) if (typeof query === 'string' && query.trim()) searchQueries.push(query.trim());
    }
    ...
    for (const block of step.content) {
      ...
      for (const annotation of annotations) {
        ...
        seenSources.add(source.url); sources.push(source);
      }
    }
  }
...
function parseJsonObject(text) {
  ...
  const firstBrace = withoutFence.indexOf('{'); const lastBrace = withoutFence.lastIndexOf('}');
...
export function extractProviderTokenUsage(data) {
  ...
  const inputTokens = safeTokenCount(usage.total_input_tokens ?? usage.input_tokens ?? usage.prompt_tokens);
  const outputTokens = safeTokenCount(usage.total_output_tokens ?? usage.output_tokens ?? usage.completion_tokens);
  ...
  const reasoningTokens = safeTokenCount(usage.total_thought_tokens ?? (usage.output_tokens_details && usage.output_tokens_details.reasoning_tokens) ?? (usage.completion_tokens_details && usage.completion_tokens_details.reasoning_tokens));
  const cachedTokens = safeTokenCount(usage.total_cached_tokens ?? (usage.prompt_tokens_details && usage.prompt_tokens_details.cached_tokens) ?? (usage.input_tokens_details && usage.input_tokens_details.cached_tokens));
  const toolUseTokens = safeTokenCount(usage.total_tool_use_tokens);
  if (inputTokens === null && outputTokens === null && totalTokens === null && reasoningTokens === null && cachedTokens === null && toolUseTokens === null) return null;
  return { input_tokens: inputTokens, output_tokens: outputTokens, total_tokens: totalTokens, reasoning_tokens: reasoningTokens, cached_tokens: cachedTokens, tool_use_tokens: toolUseTokens };
}
...
function parseAnswerText(rawText) {
  ...
  if (parsed) {
    ...
    return { answer, action: VALID_ACTIONS.includes(parsed.action) ? parsed.action : 'none', confidence: clampConfidence(parsed.confidence, 0.0) };
  }
  const afterThinking = raw.replace(/<think>[\s\S]*?<\/think>/gi, '').trim();
  return { answer: (afterThinking || raw).slice(0, 4000), action: 'none', confidence: 0.0 };
}

export function parseOpenAIChatResponse(data) {
  const message = data && data.choices && data.choices[0] && data.choices[0].message;
  if (!message || typeof message !== 'object') return { answer: '', action: 'none', confidence: 0.0, usedReasoning: false };
...
function mergeSources(...groups) {
  ...
  for (const group of groups) for (const source of Array.isArray(group) ? group : []) {
    if (!source || typeof source.url !== 'string' || seen.has(source.url)) continue;
    seen.add(source.url); merged.push(source);
  }
...
function mergeQueries(...groups) {
  const merged = [];
  for (const group of groups) for (const query of Array.isArray(group) ? group : []) {
...
async function fetchWithDeadline(url, init, timeoutMs) {
  ...
  const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), boundedTimeout);
  try { return await fetch(url, Object.assign({}, init, { signal: controller.signal })); }
  catch (error) { if (controller.signal.aborted || isUpstreamTimeoutError(error)) throw upstreamTimeoutError(); throw error; }
  finally { clearTimeout(timer); }
}

async function requestOpenAICompatible(config, question, currentTime, officialContext, locale, options = {}) {
  const system = options.rejectedDraft ? buildCorrectiveSystemPrompt(currentTime, officialContext, locale, options.rejectedDraft) : buildSystemPrompt(currentTime, officialContext, locale);
  const upstream = await fetchWithDeadline(config.endpoint, { method: 'POST', redirect: 'manual', headers: { 'Authorization': `Bearer ${config.key}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ model: config.model, messages: [{ role: 'system', content: system }, { role: 'user', content: question }], temperature: 0.1, max_tokens: 700 }) }, options.timeoutMs);
  if (!upstream.ok) { await upstream.text(); return { ok: false, failureCode: 'upstream_error' }; }
  let data; try { data = await upstream.json(); } catch (_) { return { ok: false, failureCode: 'malformed_response' }; }
  const parsed = parseOpenAIChatResponse(data); if (!parsed.answer) return { ok: false, failureCode: 'empty_response' };
...
async function requestGeminiInteractions(config, question, currentTime, officialContext, locale, options = {}) {
  const upstream = await fetchWithDeadline(config.endpoint, { method: 'POST', redirect: 'manual', headers: { 'x-goog-api-key': config.key, 'Content-Type': 'application/json' }, body: JSON.stringify({ model: config.model, input: buildGroundedPrompt(question, currentTime, officialContext, locale, options.rejectedDraft || ''), tools: [{ type: 'google_search' }], store: false }) }, options.timeoutMs);
  if (!upstream.ok) { await upstream.text(); return { ok: false, failureCode: 'upstream_error' }; }
  let data; try { data = await upstream.json(); } catch (_) { return { ok: false, failureCode: 'malformed_response' }; }
  const parsed = parseGroundedInteraction(data); if (!parsed.answer) return { ok: false, failureCode: 'empty_response' };
  const sources = mergeSources(officialContext.sources, parsed.sources);
  const primarySource = officialContext.sourceUrl || (parsed.sources[0] && parsed.sources[0].url) || '';
  return { ok: true, answer: parsed.answer, action: parsed.action, confidence: parsed.confidence, freshnessState: officialContext.freshnessState, sources, sourceUrl: primarySource, searchQueries: mergeQueries(officialContext.searchQueries, parsed.searchQueries), usedReasoning: false, tokenUsage: extractProviderTokenUsage(data) };
}

async function requestProvider(config, question, currentTime, officialContext, locale, options = {}) {
  if (config.provider === 'gemini' && config.apiStyle === 'interactions') return requestGeminiInteractions(config, question, currentTime, officialContext, locale, options);
...
export async function onRequest(context) {
  ...
  if (!ingress.ok) {
    const ingressFailure = failurePayload('', primaryConfig.provider, primaryConfig.model, ingress.failureCode || 'invalid_input', retrievedAt, currentTime, 'ko');
  ...
  if (!shape.ok) {
    if (shape.reason === 'missing_question') return jsonResponse(withRuntimeMeta({ ok: false, error: 'Missing question' }), 400, headers);
    return jsonResponse(withRuntimeMeta(Object.assign(failurePayload('', primaryConfig.provider, primaryConfig.model, 'invalid_input', retrievedAt, currentTime, requestLocale), { answer: localizedFailureAnswer(requestLocale, 'invalid_input') })), shape.status || 200, headers);
  }
  ...
  if (runtimeMode.mode === 'snapshot_only') {
    const payload = failurePayload(question, primaryConfig.provider, primaryConfig.model, 'snapshot_only', retrievedAt, currentTime, requestLocale);
  ...
  function setEvidencePolicyMeta(decision) {
    ...
    evidencePolicyMeta = { version: EVIDENCE_POLICY_VERSION, decision: EVIDENCE_DECISIONS.includes(value.decision) ? value.decision : 'not_assessed', evidence_level: EVIDENCE_LEVELS.includes(value.evidenceLevel) ? value.evidenceLevel : 'model_only', signal_kinds: Array.isArray(value.signalKinds) ? value.signalKinds.filter((kind) => CONCRETE_SIGNAL_KINDS.includes(kind)).slice(0, 5) : [], reason: EVIDENCE_REASONS.includes(value.reason) ? value.reason : 'not_assessed' };
  }

  function evidenceRequiredPayload(config, decision) {
    setEvidencePolicyMeta(decision);
    const payload = failurePayload(question, config.provider, config.model, 'evidence_required', retrievedAt, currentTime, requestLocale);
  ...
  for (let index = 0; index < providerOrder.length; index += 1) {
    ...
    let result;
    try { result = await requestProvider(config, question, currentTime, officialContext, requestLocale, { timeoutMs: primaryAttemptTimeout }); }
    catch (error) { result = { ok: false, failureCode: isUpstreamTimeoutError(error) ? 'upstream_timeout' : 'upstream_error' }; }
    ...
    if (assessment.ok) {
      ...
      if (!evidenceDecision.ok) {
        ...
        return jsonResponse(withRuntimeMeta(evidenceRequiredPayload(config, evidenceDecision)), 200, headers);
      }
    ...
    if (correctionBudget > 0) {
      ...
      let corrected;
      try { corrected = await requestProvider(config, question, currentTime, officialContext, requestLocale, { rejectedDraft, timeoutMs: correctionAttemptTimeout }); }
      catch (error) { corrected = { ok: false, failureCode: isUpstreamTimeoutError(error) ? 'upstream_timeout' : 'upstream_error' }; }
